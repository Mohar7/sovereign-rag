...
Local-first GraphRAG: Milvus hybrid retrieval (dense + BM25 + RRF) ‖ Neo4j knowledge-graph local-search → cross-encoder reranking → LangGraph orchestration with HITL on web fallback.
...
3. For codebase context, read `wiki/modules/sovereign_rag.agent.md` (and siblings as they are added)
...
## Entry points
- API: `src/sovereign_rag/api.py` (FastAPI)
- Graph: `src/sovereign_rag/agent/graph.py:graph` (referenced from `langgraph.json`)
